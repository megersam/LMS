...
export const addAnswer = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { answer, courseId, contentId, questionId }: IAddAnswerDate =
        req.body;
      const course = await CourseModel.findById(courseId);
...
export const addReview = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!courseExist) {
        return next(
          new ErrorHandler("You are not eligible to write review", 400)
        );
      }
      ...
      const html = await ejs.renderFile(
        path.join(__dirname, "../mails/review.ejs"),
        data
      );
...
export const addReplyData = CatchAsyncErrors(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
